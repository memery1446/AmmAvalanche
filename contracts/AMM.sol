...
contract AMM{
	...
	function addLiquidity(uint256 _token1Amount, uint256 _token2Amount) external {
		// Deposit tokens
		require(
			token1.transferFrom(msg.sender, address(this), _token1Amount), 'failed to transfer token 1'
			);

		require(
			token2.transferFrom(msg.sender, address(this), _token2Amount), 'failed to transfer token 2'
			);
	...
	// Determine how many token2 tokens must be deposited when depositing liq. for token1
	function calculateToken2Deposit(uint256 _token1Amount) public view returns (uint256 token2Amount) {
		token2Amount = (token2Balance * _token1Amount) / token1Balance;
	}

	// Determine how many token1 tokens must be deposited when depositing liq. for token2
	...
	function swapToken1(uint256 _token1Amount) external returns(uint256 token2Amount){
		...
		// Do Swap
			...
			// 1. Transfer tokens out of user wallet // we'll call transferFrom from the ERC20 contract //
		token1.transferFrom(msg.sender, address(this), _token1Amount);
			// 2. Update the token1 balance in the contract
		token1Balance += _token1Amount;
			// 3. Update the token2 balance in the contract
		token2Balance -= token2Amount; 
			// 4. Transfer token2 tokens from contract to user wallet // no approval needed //
  ...
  function removeLiquidity(uint256 _share) external returns (uint256 token1Amount, uint256 token2Amount) {
  		...
  		(token1Amount, token2Ammount) = calculateWithdrawAmount(_share);
